	...
	MinimalDifference(DifferenceSharedPtr difference) {
		...
		for (const auto& modification : difference->getModifications()) {
			...
			case Difference::MOVE_BLOCK:
				...
				addMovedBlock(std::get<0>(moveModification), std::get<1>(moveModification));
				break;
...
private:
	...
	void addRemovedConnection(Position outputPosition, Position inputPosition) { modifications.push_back({ REMOVED_CONNECTION, std::make_pair(outputPosition, inputPosition) }); }
	void addCreatedConnection(Position outputPosition, Position inputPosition) { modifications.push_back({ CREATED_CONNECTION, std::make_pair(outputPosition, inputPosition) }); }
