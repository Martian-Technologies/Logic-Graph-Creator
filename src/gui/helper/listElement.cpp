...
// ListElement::ListElement(Rml::ElementDocument* document, Rml::Element* root, GeneratorFunction generatorFunction) : document(document), root(root), generatorFunction(generatorFunction) {
// 	if (root->GetNumChildren() != 0) {
// 		logWarning("ListElement root should be empty when creating ListElement. This can lead to unexpected behavior.", "ListElement");
// 	}
// 	root->SetClass("menutree", true);
// 	Rml::ElementPtr list = document->CreateElement("ul");
// 	root->AppendChild(std::move(list));
// }

// void ListElement::setItems(const std::vector<std::string>& items) {
	
// }
// void ListElement::appendItem(const std::string& item) {
// 	Rml::ElementPtr listItem = document->CreateElement("li");
// 	void* data = generatorFunction(item, listItem.get(), nullptr);
// 	listItems.emplace_back(item, data);
// }
// void ListElement::insertItem(unsigned int index, const std::string& item) { }
// void ListElement::removeIndex(unsigned int index) { }
// void ListElement::getIndex(unsigned int index) { }
...
// 		else {
// 			newItem->AddEventListener("click", new EventPasser([this](Rml::Event& event) {
// 										  event.StopPropagation();
// 										  Rml::Element* target = event.GetCurrentElement();
// 										  if (listenerFunction) listenerFunction(target->GetId().substr(0, target->GetId().size() - 5));
// 									  }));
// 		}
// 		// Unified selection handling (applies .selected to clicked li, clears others). Runs in addition to any existing click callback.
// 		newItem->AddEventListener("click", new EventPasser([](Rml::Event& e) {
...
// 		newItem->AddEventListener("mouseover", new EventPasser([](Rml::Event& e) {
// 									  Rml::Element* current = e.GetCurrentElement();
...
// 								  }));
// 		newItem->AddEventListener("mouseout", new EventPasser([](Rml::Event& e) {
// 									  Rml::Element* current = e.GetCurrentElement();
// 									  Rml::Element* target = e.GetTargetElement();
// 									  if (!current || !target) return;
...
// 											  root->GetElementsByTagName(rows, "li");
// 											  for (auto* r : rows)
// 												  if (r != parentLi) r->SetClass("hovered-leaf", false);
// 										  }
