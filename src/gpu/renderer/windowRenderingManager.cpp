#include "windowRenderingManager.h"

WindowRenderingManager::WindowRenderingManager(SdlWindow* sdlWindow)
	: sdlWindow(sdlWindow) {
	...
	subrenderer = std::make_unique<WindowRenderer>(swapchain.get(), frames);
	swapchain->createFramebuffers(subrenderer->getRenderPass());
	
	// start render loop
	running = true;
	renderThread = std::thread(&WindowRenderingManager::renderLoop, this);
}

WindowRenderingManager::~WindowRenderingManager() {
...
void WindowRenderingManager::resize(std::pair<uint32_t, uint32_t> windowSize) {
...
void WindowRenderingManager::renderLoop() {
	while(running) {
...
void WindowRenderingManager::recreateSwapchain() {
	vkDeviceWaitIdle(device);
...
void WindowRenderingManager::prepareForRml(RmlRenderInterface& renderInterface) {
